...
- **2026-07-25** — Restored public profile chat after production verification
  ...
  a full public-memory retry through Karte's product free-ai gateway when a
  user-configured provider fails, then compact public memory and a
  deterministic public-bio fallback if neither provider can complete.
