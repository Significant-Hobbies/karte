...
custom edge worker in front. Local development and production use the same D1
schema; production also uses R2 bindings.
...
Browser ──► Cloudflare edge
         ...
         OpenNext handler (.open-next/worker.js)
              │
              ├─ D1  (linkchat-auth)   → pages, links, projects, chat, analytics,
              │                          creator opportunities, better-auth
...
| Database | local D1 via Wrangler | Cloudflare D1 `linkchat-auth` |
...
- `DB` → D1 `linkchat-auth` (app and better-auth tables).
...
- Smart Placement is enabled for the Worker.

## Deep dives

- Edge worker + routing: `docs/architecture/edge-worker.md`
- Data layer (D1 + R2 + schema + migrations): `docs/architecture/data.md`
