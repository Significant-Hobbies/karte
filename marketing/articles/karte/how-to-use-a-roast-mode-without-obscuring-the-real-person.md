...
slug: "how-to-use-a-roast-mode-without-obscuring-the-real-person"
target_query: "ai profile roast mode"
search_intent: "Informational - Users are exploring how to integrate AI-driven engagement features like 'roast modes' into public profiles or personal websites without undermining their professional identity or authentic personal brand."
meta_title: "How to Use a Roast Mode Without Obscuring the Real Person"
meta_description: "Learn how to integrate AI profile modes like roasts and encyclopedias while preserving your authentic public identity, creator ownership, and reliable inbound contact paths."
---

## Outline

1. **Introduction:** The tension between engaging AI profile modes and preserving professional authenticity.
2. **The Appeal and the Risk:** Why visitors enjoy interactive modes and how fragile AI can overshadow the creator.
3. **Centering Creator Ownership:** Positioning the public profile as the undeniable source of truth.
4. **Architecting for Reliability:** Utilizing bounded fallbacks, stable loading shells, and managed memory to prevent AI failures.
5. **Protecting the Inbound Path:** Ensuring that entertainment leads reliably to meaningful contact and connection.
6. **Concrete Implementation Examples:** Real-world lessons from integrating optional AI modes securely.
7. **Next Action:** Steps for independent operators to evaluate their profile's resilience.
8. **Internal-Link Suggestions:** Contextual opportunities to link related product features.
9. **Source Notes (Internal Only):** Tracing claims back to repository evidence.
...
*   Link to the platform's guide on configuring a custom domain to reinforce explicit creator ownership.
*   Link to documentation on how the managed knowledgebase parses and indexes uploaded portfolio items.
*   Link to the changelog detailing recent security enhancements to the public contact form and direct messaging paths.
*   Link to the `/ai-link-in-bio` guide for broader context on integrating AI assistants.

## Source Notes (Internal Only)

*   **PRODUCT.md:** Confirms Karte's positioning as a "public inbound assistant" and explicit constraints: "Prefer reliable, bounded fallbacks over fragile AI-only behavior," "Keep the public profile useful before adding product breadth," and "Preserve local ownership and explicit creator control." Also supports the availability of optional encyclopedia, roast, and newspaper modes.
*   **PROJECT_STATUS.md:** Confirms the implementation of Turnstile-protected contact submissions failing closed without server-side verification. Confirms bounded per-profile, per-IP limits for conversation creation via durable `RateLimiterDO`. Details the specific fallback architecture for chat generation (falling back from stale provider to free-ai gateway with full prompt, then compact prompt, then deterministic bio answer). Mentions the implementation of the stable loading shell while D1 data resolves. Confirms the lexical-only retrieval fallback for profile memory after a 150 ms timeout for semantic misses.
*   **README.md:** Reaffirms the durable nature of the `RateLimiterDO` and the use of the `knowledgebase` RAG service for profile memory.
