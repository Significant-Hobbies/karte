...
interface PingFn {
  ...
  debug: (
    event: string,
    options?: Omit<PingOptions, 'level'>,
  ) => Promise<boolean>;
  info: (
    event: string,
    options?: Omit<PingOptions, 'level'>,
...
function resolveConfig(config: PingConfig): ResolvedConfig {
  return {
    ...
    environment:
      config.environment ?? readEnv('APP_HEALTH_ENVIRONMENT') ?? 'production',
  };
}
...
export const ping: PingFn = createPing({
  onError: (err) =>
    console.warn('[ping]', err instanceof Error ? err.message : String(err)),
});
